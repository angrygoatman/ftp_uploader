...
class ResumesController < ApplicationController
    ...
    def show
       @my_resume = Resume.find(params[:id])
     end
   ...
   def create
       ...
       f = File.binread mystr
       result = f.unpack('B*')
       
       title_field = "myfile.txt"
      
      # make a text file with ftpd
       obj1 = Example::Main.new(ARGV)
      
       obj1.create_file(title_field, result)
      
      locvar = obj1.myfile_directory
      my_url = locvar + "/myfile.png"

      #a_file = ""     #unpack the binary, save to local tmp folder
       File.open(my_url, 'wb') do|f|
         f.write(result.pack("B*"))
       end
       
       
        #Dir.entries("public/uploads/tmp")[2..-1].each do |file_name|
        #    translated_urls.append("http://localhost:3000/uploads/tmp/" + "/#{URI.escape(file_name)}" + "/" + my_filename)
        #end
        
   

      if @resume.save
         redirect_to resumes_path, notice: my_url
         #render "show"
      else
         render "new"
      end
   ...
   private
      def resume_params
          params.require(:resume).permit(:name, :attachment)  #name
   end
